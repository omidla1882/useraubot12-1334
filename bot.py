...
# تنظیمات API
api_id = 25167359
api_hash = '09819653b3b07c14b4a728290bf81086'
...
async def broadcast_to_groups():
    """ارسال لیست داروها با مدیریت هوشمند FloodWait، Smart Retry و چرخش تصادفی گروه‌ها"""
    while True:
        try:
            ...
            if not groups:
                logger.info("⏳ در انتظار یافتن گروه‌ها...")
                await asyncio.sleep(10)
                continue
            
            current_time = time.time()
            
            # 📊 گرفتن گروه‌ها به ترتیب اولویت
            ...
            for group_id in priority_groups:
                try:
                    ...
                    # فاصله تصادفی - ساده و مستقیم
                    message_delay = random.randint(MESSAGE_DELAY_MIN, MESSAGE_DELAY_MAX)
                    
                    # بررسی آیا زمان کافی از آخرین پیام گذشته؟
                    if group_id in last_message_time:
                        time_since_last = current_time - last_message_time[group_id]
                    ...
                    logger.info(f"✅ پیام ارسال شد به گروه {group_id}")
                    
                    # تاخیر هوشمند
                    delay = int(random.randint(MIN_DELAY, MAX_DELAY) / rate_mult)
...
GROUP_SYSTEM_PROMPT = (
    ...
    "سبک پاسخ (حتما رعایت کن):\n"
    "• همیشه دقیقاً ۳ تا ۶ خط کامل محاوره‌ای فارسی بنویس. هر جمله کامل با فعل و نقطه یا ؟ تموم بشه. مثل حرف زدن با دوست قدیمی در گروه.\n"
    "• کلمات طبیعی: «آره»، «خودم چند بار گرفتم»، «تجربه‌ام این بود»، «معمولاً»، «بستگه داره»، «یه نکته»، «فکر کنم»، «راستی».\n"
    "• هرگز: لیست، بولت، «البته»، «حتماً»، «قطعاً»، «۱۰۰٪»، «برای سفارش به سایت»، «ادمین تماس بگیرید»، جملات تبلیغاتی مستقیم.\n"
    "• اگر دقیق نمی‌دونی: «دقیق نمیدونم ولی...» بگو + یه سوال سبک برای ادامه گفتگو بپرس.\n"
    "• هدف: کاربر کاملاً حس کنه با انسان واقعی حرف میزنه. بعد از ۲-۳ تبادل ارزشمند، خیلی طبیعی و گرم پیشنهاد بده «جزئیاتشو خصوصی بگو ببینم» یا «پیام بده راحت‌تر حرف میزنیم».\n"
    "• هرگز جواب کوتاه، ناقص، تک خطی یا بدون سوال نده. همیشه کامل فکر کن، ۳+ خط بنویس و گفتگو رو باز نگه دار."
)
